...
final class AuthManager: AuthManagerProtocol {
	...
	func register(email: String, password: String) async {
		let result = await apiManager.register(email: email.lowercased(), password: password)
		switch result {
	...
	func login(email: String, password: String) async -> Bool {
		let loginDto = await apiManager.login(email: email.lowercased(), password: password)
		switch loginDto {
	...
	func handleLoginResponse(email: String, token: String?) {
