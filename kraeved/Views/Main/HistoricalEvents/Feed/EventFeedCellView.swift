...
struct EventFeedCellView: View {
    ...
    let event: HistoricalEventBrief
    
    var body: some View {
        ZStack {
            if event.thumbnailUrl != nil {
                AsyncImage(url: event.thumbnailUrl) { phase in
                    switch phase {
                        case .empty:
                            EmptyView()
...
//    static let event = HistoricalEvent(id: 0, name: "Name", description: "Description", date: Date.now, imageUrls: [URL(string: "kaluga1")!])
//    
